...
class OccupancyMap(GridMapBase):
    ...
    def inverse_sensor_model(self, val, z):
        # no hit (max range or invalid) -> treat as free for all visited cells
        if np.isinf(z) or z <= 0.0:
            return self.p_free

        # cell is behind the hit -> no information (keep prior)
        if val > z + self.r:
            return self.p_prior

        # cell is around the hit distance -> occupied
        if abs(val - z) <= self.r:
            return self.p_occ

        # cell is in front of the hit -> free
        if val < z - self.r:
            return self.p_free
    ...
    def update_map(self, pose, scan):      
        ...
        # assume 180 degree FOV for the LiDAR
        fieldOfView = np.pi
        angleMin = -fieldOfView / 2.0
        angleStep = fieldOfView / (numBeams - 1)

        logPrior = log_odds(self.p_prior)
        startPoint = [robotX, robotY]

        for i, measDist in enumerate(scan):
            # angle of this beam in world coordinates
            beamAngle = robotTheta + angleMin + i * angleStep
            dirX = math.cos(beamAngle)
            dirY = math.sin(beamAngle)
            direction = [dirX, dirY]

            visitedCells, intersections = self.ray_traversal(startPoint, direction)

            # go over each cell that this beam passes
            for (cellY, cellX), cellDist in zip(visitedCells, intersections):

                # if we know there was a hit and this cell is far behind it, stop
                if (not np.isinf(measDist)) and (cellDist > measDist + self.r):
                    break

                # get occupancy probability for this cell
                pOcc = self.inverse_sensor_model(cellDist, measDist)

                # convert to log-odds
                logMeas = log_odds(pOcc)

                # current log-odds stored in the grid
                logOld = self.grid[cellY, cellX]

                # update rule: L_new = L_old + (l_meas - l_prior)
                logNew = logOld + (logMeas - logPrior)

                # write back
                self.grid[cellY, cellX] = logNew
...
class ReflectanceMap(GridMapBase):
    ...
    def update_map(self, pose, scan):        
        ...
        # assume 180 degree field of view, like in Part A
        fieldOfView = np.pi
        angleMin = -fieldOfView / 2.0
        angleStep = fieldOfView / (numBeams - 1)
        ...
        for i, measDist in enumerate(scan):
            ...
            # case 1: no hit (max-range beam) -> all visited cells are misses
            if np.isinf(measDist) or measDist <= 0.0:
                for cellY, cellX in visitedCells:
                    self.miss_map[cellY, cellX] += 1
                continue

            # case 2: beam hits something at distance measDist
            hitRegistered = False
            for (cellY, cellX), cellDist in zip(visitedCells, intersections):
                if hitRegistered:
                    break  # we already found the reflecting cell

                if cellDist < measDist:
                    # ray passed THROUGH this cell before the hit -> miss
                    self.miss_map[cellY, cellX] += 1
                else:
                    # first cell at or beyond the measured distance -> hit
                    self.hit_map[cellY, cellX] += 1
                    hitRegistered = True
